...
module main()
{
	...
	difference() {
		...
		MMx()U() {
			Ty(-(dr)/2-rr)cylinder(r=dr*sin(w2)/2+rr/2+rr2/2,h=h, center=1);
	hull() {	Ty((-da/2+rr)*cos(w)) Tx((da/2-rr)*sin(w))Cy(r=rr, h=h, center=1);
		Ty((-(dr/2+rr2))*cos(w2))Tx((dr/2+rr2)*sin(w2))Cy(r=rr2, h=h, center=1);
	}
		}
		}
		// Rohr weg
